...
try:
    from dynamic_modeling.tasks import StatePredTask
...
class BaseTorchSeqAgent(StatePredTask):
    """
...
class TutorBaseRNN(nn.Module):
    ...
    def step(self, inputs, state, action, lengths, hiddens=None):
        batch_size, _ = inputs.size()

        s_embs = self.state_enc(state)
        a_embs = self.action_enc(action)

        inputs = torch.cat([inputs, s_embs, a_embs], dim=1)

        if hiddens is not None:
            output, hidden = self.encoder(inputs[:, None, :], hiddens)
        else:
            output, hidden = self.encoder(inputs[:, None, :])
